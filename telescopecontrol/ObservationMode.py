...
def obs_mode(mode, az_width, el_height, *args, **kwargs):
    if mode not in ['None', 'RasterDiscrete', 'Raster', 'Lissajous', 'Pong']:
        raise ValueError("Mode '%s' not available" % mode)
    mapping_mode = ObservationMode(az_frame=az_width, el_frame=el_height, *args, **kwargs)
    mapping_mode.mode = mode
    return mapping_mode

class ObservationMode(object):
    def __init__(self, startTimeStamp=None, azGoOff=0, elGoOff=0, periodeGoOff=5, goOffSequence=["+el","-el","+az","-az"],
        ...
        Especially by making a discretee raster the 'observationDuration' devided by the points is the time in which a raster point is used.
        ...
        timeStampToCheck: object of :class: 'Timestamp'
            ...
            Because of this calculation every real targeted point has to be calculated. Otherwise it would be possible to hit a limit switch.
            It is used for :meth: 'checkAbsoluteSequenceHitsLimitSwitch'.
        
        kwargs:
            mode: string
                One of the implemented modes. Crosses can made simultaneously. They are:
            ...
            to 'Raster'
                Drive through a path in a given time. The frame is defined by 'az_frame' and 'el_frame'.
    ...
    def makeRasterDiscrete(self):
        ...
        if isinstance(self.timeStampToCheck,Timestamp):
            if self.timeStampToCheck < (self.startTimeStamp+self.observationDuration):
                i, remain = divmod(self.timeStampToCheck-self.startTimeStamp,self.observationDuration/(self.pointsAz*self.pointsEl))
                return self.azElRasterList[int(i)]
            else:
                return [0,0]
        else:
            if Timestamp() < (self.startTimeStamp+self.observationDuration):
                i, remain = divmod(Timestamp()-self.startTimeStamp,self.observationDuration/(self.pointsAz*self.pointsEl))
                return self.azElRasterList[int(i)]
            else:
                return [0,0]
        ...
        if self.firstAz:
            for pEl in range(self.pointsEl):
                for pAz in range(self.pointsAz):
                    ...
                    self.azElRasterList.append([startAz + pAz * self.az_frame / float(self.pointsAz - 1), startEl - pEl * self.el_frame / float(self.pointsEl - 1)])
        else:
            for pAz in range(self.pointsAz):
                for pEl in range(self.pointsEl):
                    if pAz%2 == 1:
    ...
    def createRaster(self):
        ...
        self.velocityRaster = (self.rasterLines*(self.moveConstLong+self.moveConstShort)-self.moveConstShort)/float(self.observationDuration)
    ...
    def makeLissajous(self):
        ...
        '''
        if isinstance(self.timeStampToCheck,Timestamp):
            return [self.az_frame / float(2) * np.sin(np.deg2rad(self.omegaAz * (self.timeStampToCheck - self.startTimeStamp) + self.phiAz)),
                    self.el_frame / float(2) * np.sin(np.deg2rad(self.omegaEl * (self.timeStampToCheck - self.startTimeStamp) + self.phiEl))]
        else:
            return [self.az_frame / float(2) * np.sin(np.deg2rad(self.omegaAz * (Timestamp() - self.startTimeStamp) + self.phiAz)),
                    self.el_frame / float(2) * np.sin(np.deg2rad(self.omegaEl * (Timestamp() - self.startTimeStamp) + self.phiEl))]
    ...
    def makePong(self):
        ...
        if isinstance(self.timeStampToCheck,Timestamp):
            az = self.startAz + self.velocity*np.cos(np.deg2rad(self.startAngle))*(self.timeStampToCheck-self.startTimeStamp)- self.az_frame / float(2) # set it to zero on the left border of the frame
            el = self.startEl + self.velocity*np.sin(np.deg2rad(self.startAngle))*(self.timeStampToCheck-self.startTimeStamp)- self.el_frame / float(2) # set it to zero on the bottom border of the frame
        else:
            az = self.startAz + self.velocity*np.cos(np.deg2rad(self.startAngle))*(Timestamp()-self.startTimeStamp)- self.az_frame / float(2) # set it to zero on the left border of the frame
            el = self.startEl + self.velocity*np.sin(np.deg2rad(self.startAngle))*(Timestamp()-self.startTimeStamp)- self.el_frame / float(2) # set it to zero on the bottom border of the frame
